...
static void DoFetch()
{
...

    {
        ...
        std::lock_guard<std::mutex> lock(dedigamer::g_state.mtx);
        dedigamer::g_state.error = "Fetch exception";
        dedigamer::g_state.fetching = false;
        dedigamer::g_state.lastFetchTick = GetTickCount();
    }
}

static void DoFetchSehError()
{
    std::lock_guard<std::mutex> lock(dedigamer::g_state.mtx);
    dedigamer::g_state.error = "Fetch crashed (SEH)";
    dedigamer::g_state.fetching = false;
    dedigamer::g_state.lastFetchTick = GetTickCount();
}

static void DoFetchSafe()
{
    __try
    {
        DoFetch();
